            ...
            if ($conexion->connect()) {
                ...
                if ($resultProyecto->num_rows > 0) {
                    ...
                    if ($resultUsuarios->num_rows > 0) {
                        ...
                        echo "<form action='procesarUsuarios.php' method='post' onsubmit='return validarFormulario()'>";
                        ...
                        while ($rowUsuario = mysqli_fetch_array($resultUsuarios)) {
                            ...
                            if (!in_array($idUsuario, $usuariosAsignados)) {
                                if ($nombreTipoUsuario != $tipoUsuarioActual) {
                                    ...
                                    if ($tipoUsuarioActual == 'Arquitecto'){
                                        echo "<h3> Arquitectos</h3>";
                                    }else{
                                        echo "<h3> Usuarios</h3>";
                                    }
                                }

                                echo "<label class='user-checkbox'>
                                        <input type='checkbox' name='usuarios[]' value='$idUsuario'>
                                        $nombreUsuario $apellidoPat $apellidoMat - Teléfono: $numTel - Email: $email - Tipo Usuario: $nombreTipoUsuario
                                    </label><br>";
                            }
                        }
